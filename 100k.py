...
async def send_visits_async(tokens, uid, server_name, target=100000):
    ...
    async with aiohttp.ClientSession(
        ...
        while success < target:
            batch = min(target - success, 10000)
            tasks = []
...
def visit_endpoint(server, uid):
    ...
    response = {
        ...
        "success_rate": f"{round((success/sent)*10000, 1)}%" if sent > 0 else "0%"
    }
